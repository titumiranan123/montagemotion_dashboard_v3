...
const HeaderForm: React.FC<IHeaderFormProps> = ({ defaultValues, onSubmit, isSubmitting ,onCancel}) => {
    const MAX_FILE_SIZE = 5 * 1024 * 1024; // 5MB
const MAX_VIDEO_SIZE = 100 * 1024 * 1024; // 100MB
const ALLOWED_FILE_TYPES = ['image/jpeg', 'image/png', 'image/webp'];
const ALLOWED_VIDEO_TYPES = ['video/mp4', 'video/quicktime'];

      const [imagePreview, setImagePreview] = useState<string | null>(defaultValues?.thumbnail || null);
      const [videoPreview, setVideoPreview] = useState<string | null>(defaultValues?.video_link || null);
      const [isUploadingImage, setIsUploadingImage] = useState(false);
      const [isUploadingVideo, setIsUploadingVideo] = useState(false);
      const [imageuploadProgress, setImageUploadProgress] = useState(0);
      const [videouploadProgress, setVideoUploadProgress] = useState(0);
      
  const { register, handleSubmit, reset,setValue,watch ,formState: { errors } } = useForm<IHeader>({
    defaultValues: defaultValues
  });

  const [selectedType, setSelectedType] = useState<"main" | "shorts" | "talking" | "podcast" | "graphic" | "advertising" | "website">(
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6  w-full max-w-7xl mt-10 bg-[#1E2939] lg:px-10 lg:py-4 px-2 py-2 grid grid-cols-1 lg:grid-cols-2 gap-10">
      <div>
        ...
        <input
          ...
          className={`mt-1 p-2 block w-full rounded-md border-gray-300 shadow-sm bg-[#101828] focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm ${errors.title ? 'border-red-500' : 'border'}`}
        />
        {errors.title && <p className="mt-1 text-sm text-red-600">{errors.title.message}</p>}
      </div>

      <div>
        <label htmlFor="description" className="block text-sm font-medium text-gray-100 rounded-lg">
          Description *
        </label>
        <textarea
          id="description"
          ...
          className={`mt-1 p-2 block bg-[#101828] w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm ${errors.description ? 'border-red-500' : 'border'}`}
        />
      ...
      <div>
        ...
        <select
          ...
          className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-[#101828] focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 ${errors.type ? 'border-red-500' : 'border'}`}
        >
