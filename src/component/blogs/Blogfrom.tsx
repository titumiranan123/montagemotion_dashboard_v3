...
const BlogForm:React.FC<BlogFormProps> = ({ initialData, onSuccess }) => {
  ...
  const onSubmit = async (data: IBlog) => {
    try {
      ...
      Swal.fire('Error!', error.response.data.errorMessages[0].message, 'error');
    }
  };
...
<div className="space-y-6">
          {/* Image Upload */}
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-1">Featured Image*</label>
            {isUploadingImage && (
              <div className="mb-4">
                ...
                <div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <label
                      ...
                      className={`flex-1  ${isUploadingImage ? 'opacity-50 pointer-events-none' : ''}`}
                    >
      ...
      <div className="flex justify-end space-x-4 pt-6 border-t">
        <button
          type="button"
          onClick={() => onSuccess?.()}
          className="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-200 hover:bg-gray-50"
        >
          Cancel
        </button>
        <button
